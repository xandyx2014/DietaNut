...
export class ComposicionPage implements OnInit {
  ...
  public async downloadAsPDF(resp) {
    ...
    autoTable(doc, {
      ...
      foot: [this.setFootSubTotal(valueStorage, "desayuno")],
    });
    autoTable(doc, {
      margin: { top: 30 },
      headStyles: {
      ...
      foot: [this.setFootSubTotal(valueStorage, "merienda")],
    });
    autoTable(doc, {
      margin: { top: 30 },
      headStyles: {
      ...
      foot: [this.setFootSubTotal(valueStorage, "almuerzo")],
    });
    autoTable(doc, {
      margin: { top: 30 },
      headStyles: {
      ...
      foot: [this.setFootSubTotal(valueStorage, "te")],
    });
    autoTable(doc, {
      margin: { top: 30 },
      headStyles: {
      ...
      foot: [this.setFootSubTotal(valueStorage, "cena")],
    });
    autoTable(doc, {
      margin: { top: 30 },
      headStyles: {
  ...
  setTotalReq(valueStorage: any) {
    ...
    return [
      ...
      ``,
      `${(
        (this.getTotalGral(valueStorage, "energia") / e.energia) *
        100
      ).toFixed(2)} %`,
      `${(
        (this.getTotalGral(valueStorage, "humedad") / e.humedad) *
        100
      ).toFixed(2)} %`,
      `${(
        (this.getTotalGral(valueStorage, "proteina") / e.proteina) *
        100
      ).toFixed(2)} %`,
      `${((this.getTotalGral(valueStorage, "grasa") / e.grasa) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "hc") / e.hc) * 100).toFixed(2)} %`,
      `${(
        (this.getTotalGral(valueStorage, "fibra_cruda") / e.fibra_cruda) *
        100
      ).toFixed(2)} %`,
      `${((this.getTotalGral(valueStorage, "ceniza") / e.ceniza) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "calcio") / e.calcio) * 100).toFixed(
        2
      )} %`,
      `${(
        (this.getTotalGral(valueStorage, "fosforo") / e.fosforo) *
        100
      ).toFixed(2)} %`,
      `${((this.getTotalGral(valueStorage, "hierro") / e.hierro) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "vit_a") / e.vit_a) * 100).toFixed(
        2
      )} %`,
      `${(
        (this.getTotalGral(valueStorage, "tiamina") / e.tiamina) *
        100
      ).toFixed(2)} %`,
      `${(
        (this.getTotalGral(valueStorage, "riboflavina") / e.riboflavina) *
        100
      ).toFixed(2)} %`,
      `${(
        (this.getTotalGral(valueStorage, "niacina") / e.niacina) *
        100
      ).toFixed(2)} %`,
      `${((this.getTotalGral(valueStorage, "vit_c") / e.vit_c) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "agm") / e.agm) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "agp") / e.agp) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "ags") / e.ags) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "col") / e.col) * 100).toFixed(
        2
      )} %`,
      `${(
        (this.getTotalGral(valueStorage, "potasio") / e.potasio) *
        100
      ).toFixed(2)} %`,
      `${((this.getTotalGral(valueStorage, "sodio") / e.sodio) * 100).toFixed(
        2
      )} %`,
      `${((this.getTotalGral(valueStorage, "zinc") / e.zinc) * 100).toFixed(
        2
      )} %`,
      `${(
        (this.getTotalGral(valueStorage, "vit_b12") / e.vit_b12) *
        100
      ).toFixed(2)} %`,
    ];
  }
